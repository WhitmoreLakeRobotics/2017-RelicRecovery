...
//@Autonomous(name = "Recovery-Red", group = "")  // @Autonomous(...) is the other common choice

public class RecoveryRed extends OpMode {
    ...
    public void loop() {
        ...
        if (CurrentStage == stage_50driveToFront) {
            if (robotChassis.getcmdComplete()) {
                robotChassis.cmdDrive(AUTO_DrivePower, 90, 36);
        ...
        if (CurrentStage == stage_60TurnTwo) {
            if (robotChassis.getcmdComplete()) {
                robotChassis.cmdTurn(AUTO_TurnPower, -AUTO_TurnPower, 180);
        ...
        if (CurrentStage == stage_70driveToBox) {
            if (robotChassis.getcmdComplete()) {
                robotChassis.cmdDrive(AUTO_DrivePower, 180, 36.5);
		...
		if (CurrentStage == stage_90Backup) {
            if (robotChassis.gripper.Is_Open()) {
                robotChassis.cmdDrive(-AUTO_DrivePower, 180, 1);
